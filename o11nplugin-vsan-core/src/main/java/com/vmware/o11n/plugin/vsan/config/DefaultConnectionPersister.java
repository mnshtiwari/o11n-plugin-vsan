...
public class DefaultConnectionPersister implements ConnectionPersister {
    ...
    private static final String HOST = "serviceHost";
    private static final String PORT = "servicePort";
    private static final String SSL_THUMB_PRINT = "sslThumbPrint";
    ...
    private void addConnectionInfoToConfig(IEndpointConfiguration config, ConnectionInfo info) {
        try {
            ...
            config.setString(PASSWORD, info.getPassword());
            config.setString(HOST, info.getHost());
            config.setInt(PORT, info.getPort());
            config.setString(SSL_THUMB_PRINT, info.getSslThumbPrint());
        } catch (Exception e) {
    ...
    private ConnectionInfo getConnectionInfo(IEndpointConfiguration config) {
        ...
        try {
            ...
            info = new ConnectionInfo(
                  config.getString(HOST), config.getAsInteger(PORT),
                  config.getString(SSL_THUMB_PRINT), config.getString(USERNAME),
