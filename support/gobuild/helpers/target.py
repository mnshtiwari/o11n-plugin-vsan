...
class Target:
   ...
   def GetStorageInfo(self, hosttype):
      ...
      Each dictionary must contain:
          ...
          NOTE: Apparently keep is no longer used.
      ...
      4) When storing your build directory, add the hosttype to
         ...
         builds don't collide (e.g. 'build' -> '%s/build' % hosttype )

      If you are using make or scons there are helper methods below.

      Sample Implementation for product 'foobar':
      Fixed comment since ones in gobuild-starterkit dont work.

      Note: Source can be stored only for a single hosttype
      
      if hosttype.startswith('linux'):
         store.append({'type': 'source', 'src': 'foobar'})
      store.append({'type': 'build', 'src': 'foobar/build'})
   ...
   def GetComponentDependencies(self):
      ...
      Returns a dictionary of dictionaries.  The first dictionary
      is keyed by the name of the component you depend on.  It's
      value is a 2nd dictionary which contains where to pull that
      component from:
